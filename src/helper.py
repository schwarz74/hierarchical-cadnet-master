...
def dataloader_adj_capped(file_path, cap):
    """Load dataset with only adjacency matrix information."""
    hf = h5py.File(file_path, 'r')
    ...
    for key in list(hf.keys()):
        ...
        V_2_shape = np.array(group.get("V_2")).shape
        if V_2_shape > (cap,0):
            print(f"batch №{key} is too big {V_2_shape} and therefore skipped")
            continue
...
def dataloader_edge(file_path):
    ...
    hf = h5py.File(file_path, 'r')
    with open(f"helper_log.txt", 'a') as writer:
        for key in list(hf.keys()):
            group = hf.get(key)
